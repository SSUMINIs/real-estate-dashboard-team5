...
# 탭 및 사이드바 구성
tab1, tab2, tab3 = st.tabs(["노후 건물 분포도", "건물년도 비교", "탭 3"])
...
with tab1:
    ...
    dict(
        ...
        xref='lon',
        yref='lat',
        text=name,  # 데이터프레임에서 'SIG_KOR_NM'에 해당하는 실제 값 사용
        showarrow=True,
        arrowhead=4,
        ax=0,
        ay=-40
    ) for lat, lon, name in zip(tab1_data['geometry'][:2], tab1_data['geometry'][:2], tab1_data['SIG_KOR_NM'][:])
    ]
...
with tab2:
    ...
    # selected_gu_option =  st.sidebar.selectbox("구를 선택하세요.", district_age_data['SGG_NM'].unique())
    # selected_dong_option = st.sidebar.selectbox("동을 선택하세요.", district_age_data.loc[district_age_data['SGG_NM'] == selected_gu_option]['BJDONG_NM'].unique())
    ...
    if selected_gu_option == '전체':
        ...
        selected_dong_option = st.sidebar.selectbox("동을 선택하세요.", district_age_data.loc[district_age_data['SGG_NM'] == selected_gu_option]['BJDONG_NM'].unique(), key=f"{selected_gu_option}_dong")
